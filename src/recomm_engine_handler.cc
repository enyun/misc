...
namespace recomm_engine {
    ...
    void RecommEngineHandler::query(idl::RecommendationResponse& _return, const idl::RecommendationRequest& request) {
        ...
        int ret = user_profile_storage_->GetUserProfile(request.uid, &user_profile);
        ...
        for (std::map<int64_t, int32_t>::const_iterator it = user_profile.keywords.begin(); it != user_profile.keywords.end(); ++it) {
        ...
        for(std::vector<idl::RetrievalResult>::const_iterator iter = retrieval_response.results.begin(); iter != retrieval_response.results.end(); ++iter) {
            if (user_profile.history.end() != std::find(user_profile.history.begin(), user_profile.history.end(), iter->story_id)) {
        ...
        int num_results = ranking_plugin->Rank(user_profile, candidates, &_return.results, false);
